...
export interface ChechResult {
    valid: boolean;
    reasons: PasswordErrors[];
}

export class PasswordChecker {
    public checkPassword(password: string): ChechResult {
    ...
    public checkAdminPassword(password: string): ChechResult {
    ...
    private checkForUpperCase(password: string, reasons: PasswordErrors[]) {
        if (password == password.toLocaleLowerCase()) reasons.push(PasswordErrors.NO_UPPER_CASE);
    }

    private checkForLowerCase(password: string, reasons: PasswordErrors[]) {
        if (password == password.toLocaleUpperCase()) reasons.push(PasswordErrors.NO_LOWER_CASE);
    }
